...
Transactionとは、Routeから渡された必要なパラメータを用いて業務ロジックを実行する層である。Transaction自身はパラメータの取得元や取得方法を意識せず、CRUDの組み合わせ・業務ルールの適用・Responseの構築のみを責務とする。

DBは必要最小限のカラムのみ保持する。ただし、そのテーブルの責務を果たすために必要な情報は保持する。外部テーブルから取得できる情報は原則保持しないが、対象エンティティがまだ存在しない場合（例：招待・仮登録）は、作成に必要な情報を保持してよい。
